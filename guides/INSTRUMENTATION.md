...
ActiveSupport::Notifications.subscribe("sla.exceeded.job_workflow") do |*args|
  event = ActiveSupport::Notifications::Event.new(*args)
  StatsD.increment(
    "job_workflow.sla.exceeded",
    tags: [
      "type:#{event.payload[:sla_type]}",
      "task:#{event.payload[:task_name] || 'workflow'}"
    ]
  )
end
```
