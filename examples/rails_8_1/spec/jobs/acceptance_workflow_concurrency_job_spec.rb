...
RSpec.describe AcceptanceWorkflowConcurrencyJob do
  context "when executing synchronously with workflow_concurrency configured" do
    ...
    let(:workflow_job) { described_class.new(tenant_id: 7, items: [10, 20, 30]) }
  ...
  context "when resolving concurrency key as parent job" do
    let(:workflow_job) { described_class.new(tenant_id: 42, items: [1]) }
