...
RSpec.describe JobFlow::Queue do
  let(:adapter) { JobFlow::QueueAdapters::NullAdapter.new }

  before { allow(JobFlow::QueueAdapter).to receive(:current).and_return(adapter) }
  ...
  describe ".pause" do
    ...
    before do
      ActiveSupport::Notifications.subscribe("queue.pause.job_flow") do |*args|
        events << ActiveSupport::Notifications::Event.new(*args)
      end
    end

    after { ActiveSupport::Notifications.unsubscribe("queue.pause.job_flow") }
  ...
  describe ".resume" do
    ...
    before do
      ...
      ActiveSupport::Notifications.subscribe("queue.resume.job_flow") do |*args|
        events << ActiveSupport::Notifications::Event.new(*args)
      end
    end

    after { ActiveSupport::Notifications.unsubscribe("queue.resume.job_flow") }
  ...
  describe ".workflows" do
    ...
    after do
      JobFlow::DSL._included_classes.delete(import_job_class)
      JobFlow::DSL._included_classes.delete(export_job_class)
      JobFlow::DSL._included_classes.delete(batch_job_class)
    end
