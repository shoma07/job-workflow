...
RSpec.describe JobFlow::Instrumentation::LogSubscriber do
  let(:subscriber) { described_class.new }
  let(:logger) { instance_double(ActiveSupport::Logger) }

  before do
    allow(JobFlow).to receive(:logger).and_return(logger)
  ...
  describe "#workflow_start" do
    ...
    it "logs at info level" do
      call
      expect(logger).to have_received(:info).with(hash_including(event: "workflow.start.job_flow"))
    end
  end
  ...
  describe "#workflow_complete" do
    ...
    it "logs at info level" do
      call
      expect(logger).to have_received(:info).with(hash_including(event: "workflow.complete.job_flow"))
    end
  end
  ...
  describe "#task_error" do
    ...
    it "logs at error level" do
      call
      expect(logger).to have_received(:error).with(hash_including(event: "task.error.job_flow"))
    end
  ...
  describe "#task_retry" do
    ...
    it "logs at warn level" do
      call
      expect(logger).to have_received(:warn).with(hash_including(event: "task.retry.job_flow"))
    end
  ...
  describe "#throttle_acquire_start" do
    ...
    let(:event) do
      ActiveSupport::Notifications::Event.new(
        "throttle.acquire.start.job_flow",
        Time.current,
        Time.current,
        "transaction_id",
        { concurrency_key: "test_key", concurrency_limit: 5 }
  ...
  describe "#throttle_acquire_complete" do
    ...
    let(:event) do
      ActiveSupport::Notifications::Event.new(
        "throttle.acquire.complete.job_flow",
        Time.current,
        Time.current + 0.5,
        "transaction_id",
        { concurrency_key: "test_key", concurrency_limit: 5 }
  ...
  describe "#dependent_wait_complete" do
    ...
    let(:event) do
      ActiveSupport::Notifications::Event.new(
        "dependent.wait.complete.job_flow",
        Time.current,
        Time.current,
        "transaction_id",
        { job_id: "job-123", dependent_task_name: :dep_task }
    ...
    it "logs at debug level" do
      call
      expect(logger).to have_received(:debug).with(hash_including(event: "dependent.wait.complete.job_flow"))
    end
  end
  ...
  describe "#task_start" do
    ...
    it "logs at info level" do
      call
      expect(logger).to have_received(:info).with(hash_including(event: "task.start.job_flow"))
    end
  end
  ...
  describe "#task_complete" do
    ...
    it "logs at info level" do
      call
      expect(logger).to have_received(:info).with(hash_including(event: "task.complete.job_flow"))
    end
  end
