...
RSpec.describe JobWorkflow::AutoScaling::Executor do
  describe "#update_desired_count" do
    ...
    before do
      allow(JobWorkflow::AutoScaling::Adapter).to receive(:fetch).with(:aws).and_return(adapter.class)
      allow(adapter.class).to receive(:new).and_return(adapter)
      allow(adapter).to receive(:update_desired_count)
    end

    shared_context "with stub JobWorkflow::Queue.latency" do
      before do
        allow(JobWorkflow::Queue).to receive(:latency).with("default").and_return(1800)
      end
    end

    context "when queue latency is nil" do
      before do
        allow(JobWorkflow::Queue).to receive(:latency).with("default").and_return(nil)
      end
    ...
    context "when queue latency is less than max latency and step_count is 1" do
      include_context "with stub JobWorkflow::Queue.latency"
    ...
    context "when queue latency is grater than max_latency" do
      ...
      before do
        allow(JobWorkflow::Queue).to receive(:latency).with("default").and_return(3_600)
      end
