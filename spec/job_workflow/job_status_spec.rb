...
RSpec.describe JobWorkflow::JobStatus do
  ...
  describe "#initialize" do
    ...
    context "with task_job_statuses" do
      let(:arguments) do
        {
          task_job_statuses: [
            JobWorkflow::TaskJobStatus.new(task_name: :task_a, job_id: "job1", each_index: 0),
            JobWorkflow::TaskJobStatus.new(task_name: :task_a, job_id: "job2", each_index: 1)
          ]
        }
      end
    ...
    context "when task exists" do
      let(:task_job_statuses) do
        [
          ...
          JobWorkflow::TaskJobStatus.new(task_name: :task_b, job_id: "job3", each_index: 0, status: :running)
        ]
      end
  ...
  describe "#finished_job_ids" do
    ...
    context "when some jobs are finished" do
      let(:task_job_statuses) do
        [
          JobWorkflow::TaskJobStatus.new(task_name: :task_a, job_id: "job1", each_index: 0, status: :succeeded),
          JobWorkflow::TaskJobStatus.new(task_name: :task_a, job_id: "job2", each_index: 1, status: :pending),
          JobWorkflow::TaskJobStatus.new(task_name: :task_a, job_id: "job3", each_index: 2, status: :failed)
        ]
      end
    ...
    context "when no jobs are finished" do
      let(:task_job_statuses) do
        [
          JobWorkflow::TaskJobStatus.new(task_name: :task_a, job_id: "job1", each_index: 0, status: :pending),
          JobWorkflow::TaskJobStatus.new(task_name: :task_a, job_id: "job2", each_index: 1, status: :running)
        ]
      end
  ...
  describe "#flat_task_job_statuses" do
    ...
    let(:job_status) do
      described_class.new(
        task_job_statuses: [
          JobWorkflow::TaskJobStatus.new(task_name: :task_a, job_id: "job1", each_index: 0),
          JobWorkflow::TaskJobStatus.new(task_name: :task_b, job_id: "job2", each_index: 0),
          JobWorkflow::TaskJobStatus.new(task_name: :task_a, job_id: "job3", each_index: 1)
        ]
      )
    end
  ...
  describe "#needs_waiting?" do
    ...
    context "when all statuses are finished" do
      ...
      let(:task_job_statuses) do
        [
          JobWorkflow::TaskJobStatus.new(task_name: :task_a, job_id: "job1", each_index: 0, status: :succeeded),
          JobWorkflow::TaskJobStatus.new(task_name: :task_a, job_id: "job2", each_index: 1, status: :failed)
        ]
      end
  ...
  describe "#update_task_job_status" do
    ...
    let(:task_job_status) do
      JobWorkflow::TaskJobStatus.new(task_name: :task_a, job_id: "job1", each_index: 0, status: :running)
    end
  ...
  describe "#update_task_job_statuses_from_db" do
    ...
    context "when some jobs are still running" do
      let(:previous_task_job_statuses) do
        [
          JobWorkflow::TaskJobStatus.new(task_name: :my_task, job_id: "job1", each_index: 0, status: :pending),
          JobWorkflow::TaskJobStatus.new(task_name: :my_task, job_id: "job2", each_index: 1, status: :pending)
        ]
      end
    ...
    context "when all jobs are already finished" do
      let(:previous_task_job_statuses) do
        [
          JobWorkflow::TaskJobStatus.new(task_name: :my_task, job_id: "job1", each_index: 0, status: :succeeded),
          JobWorkflow::TaskJobStatus.new(task_name: :my_task, job_id: "job2", each_index: 1, status: :succeeded)
        ]
      end
