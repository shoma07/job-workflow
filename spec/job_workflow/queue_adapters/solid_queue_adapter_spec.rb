...
RSpec.describe JobWorkflow::QueueAdapters::SolidQueueAdapter do
  ...
  describe "#semaphore_wait" do
    ...
    let(:semaphore) { JobWorkflow::Semaphore.new(concurrency_key: "test", concurrency_duration: 1.minute) }
  ...
  describe "#reschedule_job" do
    ...
    let(:active_job) do
      ...
      job = klass.new
      allow(job).to receive(:serialize).and_return({ "arguments" => [{ "_job_workflow_context" => {} }] })
      job
    end
