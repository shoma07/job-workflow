...
RSpec.describe JobWorkflow::Instrumentation do
  ...
  shared_context "with semaphore double" do
    let(:semaphore) do
      instance_double(JobWorkflow::Semaphore, concurrency_key: "test_key", concurrency_limit: 5)
    end
  end
  ...
  describe "::NAMESPACE" do
    ...
    it { is_expected.to eq "job_workflow" }
  end
  ...
  describe ".instrument_workflow" do
    ...
    it "fires workflow.job_workflow event (for tracing)" do
      expect(capture_events).to have_attributes(size: 1)
    end

    # rubocop:disable RSpec/ExampleLength,RSpec/MultipleExpectations
    it "fires workflow.start.job_workflow and workflow.complete.job_workflow events (for logging)" do
      events = []
      ...
      expect(events.first.first).to eq "workflow.start.job_workflow"
      expect(events.last.first).to eq "workflow.complete.job_workflow"
    ensure
  ...
  describe ".instrument_task" do
    subject(:call) do
      described_class.instrument_task(
        ...
        instance_double(
          JobWorkflow::Task,
          task_name: :my_task,
          task_retry: instance_double(JobWorkflow::TaskRetry, count: 3)
        ),
        instance_double(
          JobWorkflow::Context,
          _task_context: instance_double(JobWorkflow::TaskContext, index: 0, retry_count: 0)
        )
      ) { "task_result" }
    end
    ...
    it "fires task.job_workflow event (for tracing)" do
      expect(capture_events).to have_attributes(size: 1)
    end

    # rubocop:disable RSpec/ExampleLength,RSpec/MultipleExpectations
    it "fires task.start.job_workflow and task.complete.job_workflow events (for logging)" do
      events = []
      ...
      call
      expect(events).to have_attributes(size: 2)
      expect(events.first.first).to eq "task.start.job_workflow"
      expect(events.last.first).to eq "task.complete.job_workflow"
    ensure
      ActiveSupport::Notifications.unsubscribe(start_sub)
      ActiveSupport::Notifications.unsubscribe(complete_sub)
    end
  ...
  describe ".notify_task_skip" do
    ...
    let(:task) { instance_double(JobWorkflow::Task, task_name: :skipped_task) }
    let(:event_name) { described_class::Events::TASK_SKIP }

    it "fires a task.skip.job_workflow event" do
  ...
  describe ".notify_task_enqueue" do
    ...
    let(:task) { instance_double(JobWorkflow::Task, task_name: :enqueued_task) }
    let(:event_name) { described_class::Events::TASK_ENQUEUE }

    it "fires a task.enqueue.job_workflow event" do
  ...
  describe ".notify_task_retry" do
    subject(:call) do
      described_class.notify_task_retry(
        ...
        instance_double(
          JobWorkflow::Context,
          _task_context: instance_double(JobWorkflow::TaskContext, index: 1)
        ),
        "job-456",
        2,
        4.567,
  ...
  describe ".instrument_dependent_wait" do
    ...
    let(:task) { instance_double(JobWorkflow::Task, task_name: :dependent_task) }
    ...
    it "fires dependent.wait.start, dependent.wait, and dependent.wait.complete events" do
      ...
      expect(events[0].first).to eq "dependent.wait.start.job_workflow"
      expect(events[1].first).to eq "dependent.wait.job_workflow"
      expect(events[2].first).to eq "dependent.wait.complete.job_workflow"
    ensure
      ActiveSupport::Notifications.unsubscribe(start_sub)
  ...
  describe ".notify_dependent_reschedule" do
    ...
    it "fires a dependent.reschedule.job_workflow event" do
      expect(capture_events).to have_attributes(
        size: 1,
  ...
  describe ".instrument_throttle" do
    ...
    it "fires throttle.acquire.start, throttle.acquire, and throttle.acquire.complete events" do
      ...
      call
      expect(events).to have_attributes(size: 3)
      expect(events[0].first).to eq "throttle.acquire.start.job_workflow"
      expect(events[1].first).to eq "throttle.acquire.job_workflow"
      expect(events[2].first).to eq "throttle.acquire.complete.job_workflow"
    ensure
      ActiveSupport::Notifications.unsubscribe(start_sub)
  ...
  describe ".notify_throttle_release" do
    ...
    it "fires a throttle.release.job_workflow event" do
      expect(capture_events).to have_attributes(size: 1)
    end
  end
  ...
  describe ".instrument_custom" do
    ...
    let(:event_name) { "my_operation.job_workflow" }
  ...
  describe ".instrument_dry_run" do
    ...
    context "when dry_run is true" do
      subject(:call) do
        ctx = instance_double(JobWorkflow::Context, _task_context: task_context)
        described_class.instrument_dry_run(job, ctx, :payment_processing, 0, true) { "result" }
      end

      let(:task_context) { instance_double(JobWorkflow::TaskContext, task:, index: 0) }
      let(:task) { instance_double(JobWorkflow::Task, task_name: :my_task) }
      ...
      it "fires dry_run.skip.job_workflow event" do
        expect(capture_events).to have_attributes(size: 1)
      end
    end
    ...
    context "when dry_run is false" do
      ...
      it "fires dry_run.execute.job_workflow event" do
        expect(capture_events).to have_attributes(size: 1)
      end
    end
    ...
    context "when task is nil" do
      ...
      let(:task_context) { instance_double(JobWorkflow::TaskContext, task: nil, index: 0) }
