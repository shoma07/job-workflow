...
RSpec.describe JobWorkflow::Runner do
  describe ".new" do
  ...
  describe "#run" do
      ...
      it "raises SlaExceededError with execution sla_type" do
        expect { run }.to raise_error(
          an_instance_of(JobWorkflow::SlaExceededError)
            .and(have_attributes(sla_type: :execution, limit: 0.1))
        )
      end
    end
    ...
    context "when workflow-level execution SLA is already exceeded before run" do
      ...
      it "raises SlaExceededError immediately" do
        expect { run }.to raise_error(
          an_instance_of(JobWorkflow::SlaExceededError)
            .and(have_attributes(sla_type: :execution, limit: 0.01))
        )
      end
    end
    ...
    context "when workflow-level SLA propagates to task (no task-level override)" do
      ...
      it "raises SlaExceededError via task execution (inherited from workflow)" do
        expect { run }.to raise_error(JobWorkflow::SlaExceededError)
      end
    end
    ...
    context "when workflow-level queue wait SLA is exceeded before start" do
      let(:job) do
        klass = Class.new(ActiveJob::Base) do
          include JobWorkflow::DSL
      ...
      before do
        allow(adapter).to receive(:find_job).with(job.job_id).and_return(
          "enqueued_at" => 1.second.ago.to_f,
          "scheduled_at" => nil
        )
      end

      it "raises queue_wait SlaExceededError" do
        expect { run }.to raise_error(
          an_instance_of(JobWorkflow::SlaExceededError)
            .and(have_attributes(sla_type: :queue_wait, limit: 0.1))
        )
      end
    end

    context "when queue wait is measured from scheduled_at when present" do
      let(:job) do
        klass = Class.new(ActiveJob::Base) do
          include JobWorkflow::DSL
      ...
      before do
        allow(adapter).to receive(:find_job).with(job.job_id).and_return(
          ...
          "scheduled_at" => 0.2.seconds.ago.iso8601
        )
      end

      it "does not raise queue wait SLA error" do
        expect { run }.not_to raise_error
      end
    end

    context "when queue wait timestamp format is invalid" do
      let(:job) do
        klass = Class.new(ActiveJob::Base) do
          include JobWorkflow::DSL
      ...
      before do
        allow(adapter).to receive(:find_job).with(job.job_id).and_return(
          "enqueued_at" => "2024-13-40T99:99:99Z",
          "scheduled_at" => nil
        )
      end

      it "skips queue wait SLA check without raising" do
        expect { run }.not_to raise_error
      end
    end

    context "when adapter cannot provide current job metadata" do
      let(:job) do
        klass = Class.new(ActiveJob::Base) do
          include JobWorkflow::DSL
      ...
      before do
        allow(adapter).to receive(:find_job).with(job.job_id).and_return(nil)
      end
    ...
    context "when sub-job queue wait SLA is exceeded" do
      let(:job) do
        klass = Class.new(ActiveJob::Base) do
          include JobWorkflow::DSL
          ...
          task :task_with_queue_wait_sla, sla: { queue_wait: 0.1 } do |_ctx|
            nil
          end
        end
      ...
      let(:task) { job.class._workflow.fetch_task(:task_with_queue_wait_sla) }
      let(:ctx) do
        JobWorkflow::Context.new(
          job:,
          workflow: job.class._workflow,
          arguments: JobWorkflow::Arguments.new(data: {}),
          task_context: JobWorkflow::TaskContext.new(task:, parent_job_id: "parent-job-id", index: 0),
          output: JobWorkflow::Output.new,
          job_status: JobWorkflow::JobStatus.new
        )
      end
      ...
      before do
        allow(ctx).to receive(:_load_parent_task_output)
        allow(adapter).to receive(:find_job).with(job.job_id).and_return(
      ...
      it "raises queue_wait SlaExceededError for sub-job" do
        expect { run }.to raise_error(
          an_instance_of(JobWorkflow::SlaExceededError)
            .and(have_attributes(sla_type: :queue_wait, limit: 0.1))
        )
    ...
    context "with namespace tasks" do
      ...
      let(:ctx) do
        JobWorkflow::Context.from_hash({ job:, workflow: job.class._workflow })._update_arguments(job.arguments[0])
      end
    ...
    context "when namespace task uses throttle" do
      ...
      before do
        allow(JobWorkflow::Semaphore).to receive(:new).and_return(semaphore)
      end
    ...
    context "when task is set (sub-job execution)" do
      ...
      let(:ctx) do
        JobWorkflow::Context.new(
          job:,
          workflow: job.class._workflow,
          arguments: JobWorkflow::Arguments.new(data: { items: [10, 20] }),
          task_context: JobWorkflow::TaskContext.new(task:, parent_job_id: "parent-job-id", index: 1, value: 20),
          output: JobWorkflow::Output.new,
          job_status: JobWorkflow::JobStatus.new
        )
      end
      let(:parent_workflow_status) do
        parent_ctx = JobWorkflow::Context.new(
          workflow: job.class._workflow,
          arguments: JobWorkflow::Arguments.new(data: { items: [10, 20] }),
          task_context: JobWorkflow::TaskContext.new,
          output: JobWorkflow::Output.new,
        ...
        instance_double(JobWorkflow::WorkflowStatus, context: parent_ctx)
      end
    ...
    context "with simple tasks" do
      ...
      let(:ctx) do
        JobWorkflow::Context.from_hash({ job:, workflow: job.class._workflow })._update_arguments({ a: 0 })
      end
    ...
    context "when task has each option" do
      ...
      let(:ctx) do
        JobWorkflow::Context.from_hash({ job:, workflow: job.class._workflow })._update_arguments({ items: [1, 2, 3] })
      end
      ...
      let(:job) do
        klass = Class.new(ActiveJob::Base) do
          ...
          task :process_items, output: { result: "Integer" }, each: ->(ctx) { ctx.arguments.items } do |ctx|
            nested_task = JobWorkflow::Task.new(
              job_name: "TestJob",
              name: :nested,
    ...
    context "when task depends on map task without concurrency" do
      ...
      let(:ctx) do
        JobWorkflow::Context.from_hash({ job:, workflow: job.class._workflow })._update_arguments({ items: [5, 10] })
      end
    ...
    context "when resuming workflow with already finished parallel task" do
      ...
      let(:ctx) do
        ctx = JobWorkflow::Context.from_hash({ job:, workflow: job.class._workflow })
        ...
        ctx.output.add_task_output(
          JobWorkflow::TaskOutput.new(task_name: :parallel_work, each_index: 0, data: { result: 5 })
        )
        ctx.output.add_task_output(
          JobWorkflow::TaskOutput.new(task_name: :parallel_work, each_index: 1, data: { result: 10 })
        )
    ...
    context "when task has retry configuration" do
      ...
      let(:ctx) do
        JobWorkflow::Context.from_hash({ job:, workflow: job.class._workflow })._update_arguments({ value: 10 })
      end
    ...
    context "when task has before hooks" do
      ...
      let(:ctx) do
        JobWorkflow::Context.from_hash({ job:, workflow: job.class._workflow })._update_arguments({ value: 1 })
      end
    ...
    context "when task has around hooks" do
      ...
      let(:job) do
        ...
        klass = Class.new(ActiveJob::Base) do
          ...
          # NOTE: This is JobWorkflow's DSL around hook, not RSpec's around block
    ...
    context "when around hook does not call task.call" do
      ...
      it "raises TaskCallable::NotCalledError" do
        expect { run }.to raise_error(
          JobWorkflow::TaskCallable::NotCalledError,
    ...
    context "when task raises exception with task-specific error hook only" do
      ...
      let(:ctx) do
        JobWorkflow::Context.from_hash({ job:, workflow: job.class._workflow })._update_arguments({})
      end
  ...
  describe "dependency wait with reschedule" do
    ...
    let(:ctx) do
      ...
      ctx = JobWorkflow::Context.from_hash({ job:, workflow: job.class._workflow })._update_arguments({})
    ...
    context "when workflow execution SLA closes during dependency wait polling" do
      ...
      it "raises SlaExceededError from dependency polling check" do
        expect { runner.run }.to raise_error(
          an_instance_of(JobWorkflow::SlaExceededError)
            .and(have_attributes(sla_type: :execution))
        )
      end
    end
