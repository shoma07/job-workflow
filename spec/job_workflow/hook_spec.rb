...
RSpec.describe JobWorkflow::Hook do
  ...
  describe "#applies_to?" do
    ...
    context "when task_names is empty (global hook) checking any task" do
      let(:hook) { described_class.new(task_names: [], block: ->(_ctx) {}) }
    ...
    context "when task_names contains specific tasks and task_name is included" do
      let(:hook) { described_class.new(task_names: %i[task_a task_b], block: ->(_ctx) {}) }
  ...
  describe "#global?" do
    ...
    context "when task_names is not empty" do
      let(:hook) { described_class.new(task_names: [:task_a], block: ->(_ctx) {}) }
