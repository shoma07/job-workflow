...
RSpec.describe JobWorkflow::Context do
  ...
  describe "#initialize" do
    context "when all arguments are minimal" do
      subject(:init) do
        described_class.new(
          workflow:,
          arguments: JobWorkflow::Arguments.new(data: { arg_one: nil, arg_two: 1 }),
      ...
      it "creates a context with given arguments" do
        expect(init).to have_attributes(
          ...
          output: be_a(JobWorkflow::Output),
          job_status: be_a(JobWorkflow::JobStatus)
        )
      end
    end
    ...
    context "when task_context has parent_job_id (sub-job scenario)" do
      ...
      before do
        job_instance = Class.new(ActiveJob::Base) { include JobWorkflow::DSL }.new
    ...
    context "when output has task_outputs" do
      subject(:init) do
        described_class.new(
          ...
          output: JobWorkflow::Output.new(
            task_outputs: [
              JobWorkflow::TaskOutput.new(task_name: :task_one, each_index: 0, data: { result: 100 }),
              JobWorkflow::TaskOutput.new(task_name: :task_two, each_index: 0, data: { result: 200 })
            ]
          ),
          job_status: JobWorkflow::JobStatus.new
        )
    ...
    context "when job_status has task_job_statuses" do
      subject(:init) do
        described_class.new(
          ...
          job_status: JobWorkflow::JobStatus.new(
            task_job_statuses: [
              JobWorkflow::TaskJobStatus.new(task_name: :task_a, job_id: "job1", each_index: 0, status: :succeeded),
              JobWorkflow::TaskJobStatus.new(task_name: :task_a, job_id: "job2", each_index: 1, status: :pending)
            ]
          )
        )
      end
      ...
      it "creates a context with job status" do
        expect(init.job_status).to be_a(JobWorkflow::JobStatus)
      end
  ...
  describe "#serialize" do
    ...
    let(:ctx) do
      described_class.new(
        ...
        output: JobWorkflow::Output.new(
          task_outputs: [
            JobWorkflow::TaskOutput.new(task_name: :task_a, each_index: 0, data: { result: 100 })
          ]
        ),
        job_status: JobWorkflow::JobStatus.new(
          task_job_statuses: [
    ...
    context "when task_outputs contain a namespaced task_name" do
      let(:ctx) do
        described_class.new(
          ...
          output: JobWorkflow::Output.new(
            task_outputs: [
              JobWorkflow::TaskOutput.new(task_name: :"ns:task_one", each_index: 0, data: { result: 100 })
            ]
          ),
          job_status: JobWorkflow::JobStatus.new
        )
  ...
  describe "#arguments" do
    ...
    it do
      expect(arguments).to have_attributes(class: JobWorkflow::Arguments, arg_one: nil, arg_two: 1)
    end
  end
    ...
    context "when enabled and task is set" do
      let(:ctx) do
        described_class.new(
          workflow:,
          arguments: JobWorkflow::Arguments.new(data: {}),
          task_context: JobWorkflow::TaskContext.new(task:, parent_job_id: "019b6901-8bdf-7fd4-83aa-6c18254fe076"),
          output: JobWorkflow::Output.new,
          job_status: JobWorkflow::JobStatus.new
        )
      end
    ...
    context "when enabled and task is not set" do
      let(:ctx) do
        described_class.new(
          workflow:,
          arguments: JobWorkflow::Arguments.new(data: {}),
          task_context: JobWorkflow::TaskContext.new(parent_job_id: "019b6901-8bdf-7fd4-83aa-6c18254fe076"),
          output: JobWorkflow::Output.new,
          job_status: JobWorkflow::JobStatus.new
        )
      end
  ...
  describe "#output" do
    ...
    it "returns an Output instance" do
      expect(output).to be_a(JobWorkflow::Output)
    end
  end
    ...
    context "when dry_run? is called in each iteration" do
      let(:ctx_with_dry_run) do
        ...
        ctx = described_class.new(
          ...
          arguments: JobWorkflow::Arguments.new(data: { items: [1, 2, 3] }),
          task_context: JobWorkflow::TaskContext.new,
          output: JobWorkflow::Output.new,
          job_status: JobWorkflow::JobStatus.new
        )
  ...
  describe "#instrument" do
    ...
    context "when called within a task" do
      ...
      before do
        ActiveSupport::Notifications.subscribe(/\.job_workflow$/) do |event|
          events << event
        end
      end
      ...
      it "fires custom event" do
        instrument_result
        custom_event = events.find { |e| e.name == "api_call.job_workflow" }
    ...
    context "when called with default operation" do
      ...
      it "uses 'custom' as default operation" do
        instrument_result
        custom_event = events.find { |e| e.name == "custom.job_workflow" }
    ...
    context "when task is nil" do
      ...
      it "sets task_name to nil in payload" do
        ...
        custom_event = events.find { |e| e.name == "test_op.job_workflow" }
    ...
    context "when context is for a sub-job (has task and parent_job_id)" do
      ...
      let(:ctx) do
        context = described_class.new(
          ...
          output: JobWorkflow::Output.new(
            task_outputs: [
              JobWorkflow::TaskOutput.new(task_name: :parent_task, each_index: 0, data: { result: 100 }),
              JobWorkflow::TaskOutput.new(task_name: :child_task, each_index: 0, data: { result: 200 })
            ]
          ),
          job_status: JobWorkflow::JobStatus.new(
            task_job_statuses: [
              JobWorkflow::TaskJobStatus.new(
    ...
    context "when context has task but no parent_job_id" do
      ...
      let(:ctx) do
        context = described_class.new(
          workflow:,
          arguments: JobWorkflow::Arguments.new(data: {}),
          task_context: JobWorkflow::TaskContext.new(task: parent_task, index: 0),
          output: JobWorkflow::Output.new(
            task_outputs: [
              JobWorkflow::TaskOutput.new(task_name: :parent_task, each_index: 0, data: { result: 100 }),
              JobWorkflow::TaskOutput.new(task_name: :child_task, each_index: 0, data: { result: 200 })
  ...
  describe "#_load_parent_task_output" do
    ...
    context "when parent_job_id is present and parent exists" do
      subject(:ctx) do
        context = described_class.new(
          workflow:,
          arguments: JobWorkflow::Arguments.new(data: {}),
          task_context: JobWorkflow::TaskContext.new(parent_job_id: "parent-job-id"),
          output: JobWorkflow::Output.new,
          job_status: JobWorkflow::JobStatus.new
        )
        context._job = job
      ...
      let(:parent_workflow_status) do
        instance_double(
          ...
          context: described_class.new(
            ...
            output: JobWorkflow::Output.new(
              task_outputs: [
                JobWorkflow::TaskOutput.new(task_name: :parent_task, each_index: 0, data: { value: 42 })
              ]
            ),
            job_status: JobWorkflow::JobStatus.new(
              task_job_statuses: [
      ...
      before do
        allow(JobWorkflow::WorkflowStatus).to(
          receive(:find).with("parent-job-id").and_return(parent_workflow_status)
        )
      end
      ...
      it "loads parent context on each call" do
        ctx._load_parent_task_output
        ctx._load_parent_task_output
        expect(JobWorkflow::WorkflowStatus).to have_received(:find).twice
      end
    end
    ...
    context "when parent_job_id is nil" do
      ...
      it "does not attempt to load parent context" do
        allow(JobWorkflow::WorkflowStatus).to receive(:find)
        ctx._load_parent_task_output
        expect(JobWorkflow::WorkflowStatus).not_to have_received(:find)
      end
    end
    ...
    context "when parent_job_id is present but parent does not exist" do
      subject(:ctx) do
        context = described_class.new(
          workflow:,
          arguments: JobWorkflow::Arguments.new(data: {}),
          task_context: JobWorkflow::TaskContext.new(parent_job_id: "missing-parent"),
          output: JobWorkflow::Output.new,
          job_status: JobWorkflow::JobStatus.new
        )
        context._job = job
      ...
      before { allow(JobWorkflow::QueueAdapter.current).to receive(:find_by).with(job_id: job.job_id).and_return(nil) }

      it "does not raise an error" do
        expect { ctx._load_parent_task_output }.to raise_error(JobWorkflow::WorkflowStatus::NotFoundError)
      end
    end
  end
    ...
    context "when task_context.dry_run is true" do
      subject(:ctx) do
        described_class.new(
          ...
          workflow:,
          arguments: JobWorkflow::Arguments.new(data: {}),
          task_context: JobWorkflow::TaskContext.new(dry_run: true),
          output: JobWorkflow::Output.new,
          job_status: JobWorkflow::JobStatus.new
        )
      end
    ...
    context "when task_context.dry_run is false" do
      subject(:ctx) do
        described_class.new(
          ...
          workflow:,
          arguments: JobWorkflow::Arguments.new(data: {}),
          task_context: JobWorkflow::TaskContext.new(dry_run: false),
          output: JobWorkflow::Output.new,
          job_status: JobWorkflow::JobStatus.new
        )
      end
  ...
  describe "#skip_in_dry_run" do
    ...
    context "when not in dry-run mode" do
      subject(:ctx) do
        described_class.new(
          ...
          workflow:,
          arguments: JobWorkflow::Arguments.new(data: {}),
          task_context: JobWorkflow::TaskContext.new(task:, dry_run: false),
          output: JobWorkflow::Output.new,
          job_status: JobWorkflow::JobStatus.new
        )
      end
      ...
      around do |example|
        sub = ActiveSupport::Notifications.subscribe("dry_run.execute.job_workflow") { |*args| events << args }
        example.run
    ...
    context "when in dry-run mode" do
      subject(:ctx) do
        described_class.new(
          ...
          workflow:,
          arguments: JobWorkflow::Arguments.new(data: {}),
          task_context: JobWorkflow::TaskContext.new(task:, dry_run: true),
          output: JobWorkflow::Output.new,
          job_status: JobWorkflow::JobStatus.new
        )
      end
      ...
      around do |example|
        sub = ActiveSupport::Notifications.subscribe("dry_run.skip.job_workflow") { |*args| events << args }
        example.run
        ActiveSupport::Notifications.unsubscribe(sub)
      end
    ...
    context "with named dry_run" do
      ...
      around do |example|
        sub = ActiveSupport::Notifications.subscribe("dry_run.skip.job_workflow") { |*, p| payload.merge!(p) }
        example.run
        ActiveSupport::Notifications.unsubscribe(sub)
      end
    ...
    context "with multiple calls" do
      ...
      around do |example|
        sub = ActiveSupport::Notifications.subscribe("dry_run.skip.job_workflow") { |*, p| indices << p[:dry_run_index] }
        example.run
        ActiveSupport::Notifications.unsubscribe(sub)
      end
