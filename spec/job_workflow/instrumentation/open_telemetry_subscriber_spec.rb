...
RSpec.describe JobWorkflow::Instrumentation::OpenTelemetrySubscriber do
  ...
  describe "#start" do
    ...
    context "when OpenTelemetry is not available" do
      ...
      it "does not raise" do
        ...
        expect { subscriber.start("workflow.job_workflow", "id", payload) }.not_to raise_error
      end

      it "does not modify payload" do
        payload = { job_name: "TestJob", job_id: "123" }
        subscriber.start("workflow.job_workflow", "id", payload)
    ...
    context "when OpenTelemetry is available" do
      ...
      it "builds span name with task_name for task events" do
        ...
        subscriber.start("task.job_workflow", "id", payload)
      end
      ...
      it "uses default span name when job_name is missing" do
        payload = { job_id: "123" }
        expect(mock_tracer).to receive(:start_span).with("JobWorkflow workflow", kind: :internal, attributes: anything)
        subscriber.start("workflow.job_workflow", "id", payload)
      end
    end
  end
  ...
  describe "#finish" do
    ...
    context "when OpenTelemetry is not available" do
      ...
      it "does not raise" do
        payload = { job_name: "TestJob", job_id: "123" }
        expect { subscriber.finish("workflow.job_workflow", "id", payload) }.not_to raise_error
      end
    end
    ...
    context "when span info is present" do
      ...
      it "finishes span and detaches context" do
        ...
        subscriber.finish("workflow.job_workflow", "id", payload)
      end
    ...
    context "when extract_otel_info raises error" do
      ...
      it "rescues error in finish and still ensures cleanup" do
        ...
        expect { new_subscriber.finish("workflow.job_workflow", "id", payload) }.not_to raise_error
      end
    end
  end
  ...
  describe "Attributes module" do
    it "defines JOB_NAME constant" do
      expect(described_class::Attributes::JOB_NAME).to eq("job_workflow.job.name")
    end

    it "defines JOB_ID constant" do
      expect(described_class::Attributes::JOB_ID).to eq("job_workflow.job.id")
    end

    it "defines TASK_NAME constant" do
      expect(described_class::Attributes::TASK_NAME).to eq("job_workflow.task.name")
    end

    it "defines TASK_EACH_INDEX constant" do
      expect(described_class::Attributes::TASK_EACH_INDEX).to eq("job_workflow.task.each_index")
    end

    it "defines TASK_RETRY_COUNT constant" do
      expect(described_class::Attributes::TASK_RETRY_COUNT).to eq("job_workflow.task.retry_count")
    end

    it "defines WORKFLOW_NAME constant" do
      expect(described_class::Attributes::WORKFLOW_NAME).to eq("job_workflow.workflow.name")
    end

    it "defines ERROR_CLASS constant" do
      expect(described_class::Attributes::ERROR_CLASS).to eq("job_workflow.error.class")
    end

    it "defines ERROR_MESSAGE constant" do
      expect(described_class::Attributes::ERROR_MESSAGE).to eq("job_workflow.error.message")
    end

    it "defines CONCURRENCY_KEY constant" do
      expect(described_class::Attributes::CONCURRENCY_KEY).to eq("job_workflow.concurrency.key")
    end

    it "defines CONCURRENCY_LIMIT constant" do
      expect(described_class::Attributes::CONCURRENCY_LIMIT).to eq("job_workflow.concurrency.limit")
    end
  end
  ...
  describe "SUBSCRIBED_EVENTS" do
    ...
    it "includes workflow event" do
      expect(events).to include(JobWorkflow::Instrumentation::Events::WORKFLOW)
    end

    it "includes task event" do
      expect(events).to include(JobWorkflow::Instrumentation::Events::TASK)
    end

    it "includes throttle acquire event" do
      expect(events).to include(JobWorkflow::Instrumentation::Events::THROTTLE_ACQUIRE)
    end
