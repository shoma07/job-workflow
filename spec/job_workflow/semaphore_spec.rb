...
RSpec.describe JobWorkflow::Semaphore do
  ...
  describe ".available?" do
    context "when adapter reports semaphore unavailable" do
      before do
        allow(JobWorkflow::QueueAdapter.current).to receive(:semaphore_available?).and_return(false)
      end
    ...
    context "when adapter reports semaphore available" do
      before do
        allow(JobWorkflow::QueueAdapter.current).to receive(:semaphore_available?).and_return(true)
      end
  ...
  describe "#signal" do
    ...
    context "when adapter is available and signal succeeds" do
      before do
        ...
        allow(JobWorkflow::Instrumentation).to receive(:notify_throttle_release)
      end
      ...
      it "fires a throttle_release event" do
        signal
        expect(JobWorkflow::Instrumentation).to have_received(:notify_throttle_release).with(semaphore)
      end
    end
