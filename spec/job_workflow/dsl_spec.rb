...
RSpec.describe JobWorkflow::DSL do
  describe "#perform" do
  ...
  describe "self.task" do
    ...
    context "without options" do
      ...
      let(:ctx) { JobWorkflow::Context.from_hash({ workflow: klass._workflow })._update_arguments({ sum: 1 }) }
  ...
  describe "#serialize" do
    ...
    context "when job has not been performed" do
      it { is_expected.not_to have_key("job_workflow_context") }
    end

    context "when job has been performed" do
      before { job.perform({ value: 42 }) }

      it "includes job_workflow_context key" do
        expect(serialize).to have_key("job_workflow_context")
      end
  ...
  describe "#deserialize" do
    ...
    context "when job_data contains job_workflow_context" do
      let(:job_data) do
        {
    ...
    context "when job_data does not contain job_workflow_context" do
      let(:job_data) { {} }
